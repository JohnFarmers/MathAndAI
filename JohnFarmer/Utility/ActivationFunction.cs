...
namespace JohnFarmer.NeuralNetwork
{
	public struct ActivationFunction
	{
		...
		public static Func<double, double> SigmoidPrime = (double x) => Sigmoid(x) * (1 - Sigmoid(x));
		...
		public static Func<double, double> TanhPrime = (double x) => Math.Pow(1 / Math.Cosh(x), 2);
